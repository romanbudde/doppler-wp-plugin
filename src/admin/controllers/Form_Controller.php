...
class DPLR_Form_Controller
{
    ...
    if (isset($form) && count($form) > 0) {
      ...
      foreach ($form['fields'] as $key => $value) {
        $mod = [
              ...
              'type' => $value['type'], 
              'form_id' => $form_id, 
              'sort_order' => $field_position_counter++
        ];
  ...
  function update($form_id, $form_to_update = NULL) {
    ...
    if (isset($form_to_update) && count($form_to_update) > 0) {
      DPLR_Form_Model::update(
        $form_id, 
        ['name'=>$form_to_update['name'], 
        'title' => $form_to_update['title'], 
        'list_id' => $form_to_update['list_id']]
      );
